<template lang="">
    <nuxt-link :to="`${BASE}${link}/${item?.id}`" class="item-container flex flex-col gap-2 rounded-md hover:bg-gray-950 bg-gray-900 p-3 pb-1 w-[13rem] h-[17rem] cursor-pointer max-sm:hidden playlist-container--desktop">
                    <!--  poster -->
                    <div :class="`w-[${item?.track?.album?.images[0]?.width ?? '100%'} h-['100%'} flex-[0.95] rounded-md overflow-hidden relative`">
                        <img class="w-full h-full object-cover" :src="item?.images?.[0]?.url" />
                    </div>
                    ...
                    <div class="flex flex-col gap-2 max-sm:hidden">
                        ...
                        <div class="flex gap-1 items-center">
                            <span class="text-gray-400 text-xs">{{item?.followers?.total}}</span>
                            <span class="text-gray-400 text-xs">{{item?.popularity}}</span>
                        </div>
                    </div>
    </nuxt-link>
    <!-- Mobile -->
                <nuxt-link :to="`${BASE}${link}/${item?.track?.album?.id}`" v-for="item in items" class="item-container lg:hidden rounded-md hover:bg-gray-950 bg-gray-900 p-0 w-[13rem] h-[14rem] overflow-hidden playlist-container--mobile">
                    ...
                    <div class="w-full h-full rounded-md overflow-hidden relative">
                        <img class="w-full h-full object-cover" :src="artist?.images?.[0]?.url" />
                        <!-- Video icon -->
                        ...
                        <div class="flex gap-1 items-center">
                            <span class="text-gray-400 text-xs">{{artist?.followers?.total}}</span>
                            <span class="text-gray-400 text-xs">{{artist?.popularity}}</span>
                        </div>
                        </div>
                    </div>
                </nuxt-link>
...
onBeforeMount(() => {
    BASE.value = 'https://muse-phi.vercel.app/';
})
